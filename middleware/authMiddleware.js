...
const verifyToken = async (req, res, next) => {
    try {
        ...
        try {
            ...
            logger.warn(`Firestore user fetch warning: ${dbErr.message}`);
        }
        
        // Admin Recognition Logic
        const userEmail = (decoded.email || userData.email || '').toLowerCase();
        const isAdmin = userEmail.endsWith('@hrflow.com') || 
                        userEmail.endsWith('@kylrxai.com') ||
                        userEmail === 'admin@demo.com' ||
                        userEmail === 'nandanb449@gmail.com' ||
                        userEmail === 'info.kylrxai@gmail.com' ||
                        decoded.role === 'admin' || 
                        decoded.admin === true ||
                        (userData.role && userData.role.toLowerCase().includes('admin'));
        
        const effectiveRole = isAdmin ? 'Admin' : (userData.role || decoded.role || 'Employee');
