...
window.onerror = function (msg, url, lineNo, columnNo, error) {
    ...
    if (document.getElementById('system-fallback-ui')) return false;

    const fallbackUI = document.createElement('div');
    fallbackUI.id = 'system-fallback-ui';
    fallbackUI.innerHTML = `
        <div style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); z-index: 99999; display: flex; align-items: center; justify-content: center; color: white; font-family: 'Inter', sans-serif;">
            <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(255,255,255,0.1); border-radius: 16px; padding: 40px; max-width: 500px; text-align: center; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5);">
                <div style="font-size: 3rem; color: #ef4444; margin-bottom: 20px;">⚠️</div>
                <h2 style="margin: 0 0 10px 0; font-size: 1.5rem; font-weight: 600;">System Recovering</h2>
                <p style="color: #94a3b8; font-size: 0.95rem; line-height: 1.6; margin-bottom: 25px;">
                    HRFlow encountered an unexpected issue but our Zero-Crash architecture protected your data. We are resetting the current view.
                </p>
                <button onclick="window.location.reload()" style="background: #3b82f6; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-weight: 500; cursor: pointer; transition: background 0.2s;">
                    Reload Dashboard
                </button>
            </div>
        </div>
    ...
    return false; // Let default handler run to log to console if needed
};
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    if (userDoc.exists()) {
      ...
      // Fallback to localStorage for Demo Bypass users
      data = {
    ...
    if (data) {
      ...
      avatars.forEach(a => a.textContent = data.name.charAt(0) + (data.name.split(' ')[1]?.[0] || ''));
    }
    loadDashboardData();
  } else {
    // If not a Firebase user, check if this is a Demo or API-authenticated session
    const isLoggedIn = localStorage.getItem('hr_logged_in') === 'true';
    if (isLoggedIn) {
      console.log('✅ Authenticated via Local Session (API/Demo)');
      loadDashboardData();
    } else {
      // Not logged in at all, ensure we are at landing/login
      const currentFile = window.location.pathname.split('/').pop();
      if (currentFile && currentFile !== 'index.html' && currentFile !== 'manager-login.html' && currentFile !== 'login.html') {
          window.location.href = 'index.html';
      }
    }
  }
...
const mockEmployees = [
  { id: '1', name: 'Alice Johnson', role: 'Developer', dept: 'Engineering', status: 'Active' },
  { id: '2', name: 'Bob Smith', role: 'Designer', dept: 'Design', status: 'Active' },
  { id: '3', name: 'Charlie Brown', role: 'Manager', dept: 'Marketing', status: 'Active' },
  { id: '4', name: 'Diana Prince', role: 'Analyst', dept: 'Finance', status: 'Active' }
];

const mockAttendance = [
  { empId: '1', name: 'Alice Johnson', dept: 'Engineering', status: 'present', date: new Date().toISOString().split('T')[0] },
  { empId: '2', name: 'Bob Smith', dept: 'Design', status: 'late', date: new Date().toISOString().split('T')[0] },
  { empId: '3', name: 'Charlie Brown', dept: 'Marketing', status: 'absent', date: new Date().toISOString().split('T')[0] }
];

const mockPayroll = [
  { id: 'p1', employeeName: 'Alice Johnson', month: 'May 2026', net: '85,000', processedAt: new Date().toISOString() },
  { id: 'p2', employeeName: 'Bob Smith', month: 'May 2026', net: '72,000', processedAt: new Date().toISOString() }
];

function updateCharts() {
  const atCtx = document.getElementById('attendanceChart');
  const deptCtx = document.getElementById('deptChart');

  if (!atCtx || !deptCtx) return;

  const depts = ['Engineering', 'Marketing', 'Design', 'Finance', 'Sales', 'HR'];
  const deptCounts = depts.map(d => attendanceData.filter(e => e.dept === d).length);
  ...
  const absentCount = attendanceData.filter(e => e.status === 'absent' || e.status === 'leave').length;

  if (attendanceChartInstance) attendanceChartInstance.destroy();
  if (deptChartInstance) deptChartInstance.destroy();

  attendanceChartInstance = new Chart(atCtx, {
    type: 'bar',
    data: {
      labels: ['Mon', 'Tue', 'Wed', 'Thu', 'Today'],
      datasets: [
        { label: 'Present', data: [210, 218, 205, 214, presentCount], backgroundColor: '#3b82f6', borderRadius: 6 },
        { label: 'Absent', data: [15, 12, 18, 14, absentCount], backgroundColor: '#fee2e2', borderRadius: 6 },
        { label: 'Late', data: [23, 18, 25, 20, lateCount], backgroundColor: '#fbbf24', borderRadius: 6 },
      ]
    },
  ...
  deptChartInstance = new Chart(deptCtx, {
    ...
    data: {
      labels: depts,
      datasets: [{ 
        data: deptCounts.every(c => c === 0) ? [1,1,1,1,1,1] : deptCounts, 
...
function renderTeamList(employees) {
  ...
  if (employees.length === 0) {
    teamList.innerHTML = '<div style="text-align:center; padding:2rem; color:var(--text-muted); font-size:.9rem;">No cloud team members found.</div>';
    return;
  }
  ...
  teamList.innerHTML = employees.map(emp => `
    <div class="activity-item">
      ...
      <div class="act-body">
        ...
        <div class="act-time">${emp.role || emp.dept || 'Employee'}</div>
      </div>
    ...
    try {
        ...
        managerTaskList.innerHTML = tasks.map(task => `
            <tr>
                ...
                <td>
                    <button class="action-btn" onclick="deleteTask('${task.id}')" title="Delete Task" style="border:none; background:none; cursor:pointer; color:var(--danger);"><i data-lucide="trash-2" size="14"></i></button>
                </td>
            </tr>
        `).join('');
...
if (btnSaveTask) {
    btnSaveTask.addEventListener('click', async () => {
        ...
        try {
            ...
            // Send notification
            await sendNotification(assigneeId, 'New Task Assigned', `You have been assigned a new task: ${title}`, 'task');
            ...
            if (window.showToast) window.showToast('Task Created', 'New task assigned successfully.', 'success');
        } catch (err) {
            console.error('Error saving task:', err);
            if (window.showToast) window.showToast('Error', 'Failed to create task.', 'error');
        } finally {
...
window.deleteTask = async (id) => {
    ...
    try {
        ...
        loadTasks();
        if (window.showToast) window.showToast('Task Deleted', 'Task removed from record.', 'info');
    } catch (err) {
  ...
  try {
    ...
    tbody.innerHTML = requests.map(req => `
      <tr>
        <td>
          <div class="emp-cell">
            <div class="emp-avatar" style="background:var(--primary);">${req.userName ? req.userName.charAt(0) : '?'}</div>
        ...
        <td>
          <span class="status-pill ${req.status === 'approved' ? 'pill-present' : (req.status === 'rejected' ? 'pill-absent' : 'pill-pending')}">
...
window.handleLeaveAction = async (requestId, action) => {
  try {
    ...
    const reqSnap = await getDocs(query(collection(db, 'leave_requests'), where('__name__', '==', requestId))); // doc() doesn't give data directly for notification
    
    await updateDoc(reqRef, { status: action });
    
    // Notify employee
    const reqDoc = await getDocs(query(collection(db, 'leave_requests'), where('__name__', '==', requestId)));
    if (!reqDoc.empty) {
      const data = reqDoc.docs[0].data();
    ...
    loadLeaveRequests();
    showToast('Leave Updated', `Request has been ${action}. Notification sent.`, action === 'approved' ? 'success' : 'warning');
  } catch (err) {
...
function playNotifSound() {
  try {
    ...
    // Silent catch for browser blocking
  }
}
  ...
  if (btn.textContent.includes('Add Employee')) {
    ...
    openModal('Process Payroll', `
      <div style="text-align:center; padding:1rem 0;">
        ...
        <p>This will process salaries for all employees for <b>${new Date().toLocaleString('default', { month: 'long' })} ${new Date().getFullYear()}</b>.</p>
        <div style="margin-top:1.5rem; background:var(--bg); padding:1rem; border-radius:12px; text-align:left;">
          <div style="display:flex; justify-content:space-between; margin-bottom:.5rem;"><span>Total Employees</span><b>${attendanceData.length}</b></div>
          <div style="display:flex; justify-content:space-between;"><span>Estimated Total</span><b>₹1.2M</b></div>
        </div>
      </div>
    `, async () => {
      ...
      try {
        // Fetch all employees to send notifications
...
// ===== END OF APPLICATION =====
// Global Interaction Listener to unlock AudioContext/Play for browser policies
document.addEventListener('click', () => {
    if (window.notifSound) {
        window.notifSound.volume = 0;
        window.notifSound.play().then(() => {
            window.notifSound.pause();
            window.notifSound.volume = 1;
        }).catch(() => {});
    }
}, { once: true });
